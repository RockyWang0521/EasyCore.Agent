...
namespace AspCoreAgent.Controllers
{
    ...
    public class ElasticsearchController : ControllerBase
    {
        ...
        private readonly IElasticsearchVectorStore _elasticsearchVectorStore;
        private const string documentVector = "documentVector";

        public ElasticsearchController(QianwenAgent agent,
               DeepSeekAgent deepSeekAgent,
               IElasticsearchVectorStore elasticsearchVectorStore)
        {
            _agent = agent;
            _deepSeekAgent = deepSeekAgent;
            _elasticsearchVectorStore = elasticsearchVectorStore;
        }

        [HttpGet("ElasticsearchVectorStoreUpsert")]
        public async Task ElasticsearchVectorStoreUpsert()
        {
            var vectorCollectionDefinition = new ElasticsearchVectorCollectionDefinition
            {
                ScalarFields =
                {
            ...
            await _elasticsearchVectorStore.CreateCollectionAsync("test_collection", vectorCollectionDefinition);
...
让 AI 开发回归简单，让每一个 .NET 开发者都能轻松构建企业级 Agent 应用。
            ...
            foreach (var chunk in chunks)
            {
                ...
                var textVector = new ElasticsearchTextVector
                {
                ...
                await _elasticsearchVectorStore.UpsertAsync("test_collection", textVector);
            }
        }

        [HttpGet("ElasticsearchVectorStoreSearch")]
        public async Task<IReadOnlyList<ElasticsearchVectorSearchResult<ElasticsearchTextVector>>> ElasticsearchVectorStoreSearch()
        {
            ...
            var searchResults = await _elasticsearchVectorStore.VectorSearchAsync<ElasticsearchTextVector>(
        ...
        [HttpGet("ElasticsearchVectorStoreMmrSelector")]
        public async Task<List<MmrCandidate>> ElasticsearchVectorStoreMmrSelector()
        {
            var result = await _agent.EmbedAsync("easycore.agent支持哪些功能？");
        ...
        [HttpGet("ElasticsearchVectorStoreGet")]
        public async Task<ElasticsearchTextVector?> ElasticsearchVectorStoreGet([FromQuery] string? id)
        {
            const string collectionName = "test_collection";

            if (string.IsNullOrWhiteSpace(id))
            {
                var records = await _elasticsearchVectorStore.QueryAsync<ElasticsearchTextVector>(
                    collectionName,
                    limit: 1,
                    includeMetadata: true);
            ...
            return await _elasticsearchVectorStore.GetAsync<ElasticsearchTextVector>(
                collectionName,
                id,
                includeVector: true,
                vectorName: documentVector,
        ...
        [HttpGet("ElasticsearchVectorStoreQuery")]
        public async Task<IReadOnlyList<ElasticsearchTextVector>> ElasticsearchVectorStoreQuery()
        {
            return await _elasticsearchVectorStore.QueryAsync<ElasticsearchTextVector>(
                collectionName: "test_collection",
                filter: new ElasticsearchVectorFilter
                {
                    Conditions =
        ...
        [HttpGet("ElasticsearchVectorStoreHybridSearch")]
        public async Task<IReadOnlyList<ElasticsearchVectorSearchResult<ElasticsearchTextVector>>> ElasticsearchVectorStoreHybridSearch()
        {
            const string collectionName = "test_collection";
            var queryVector = await _agent.EmbedAsync("easycore.agent支持哪些功能？");

            var keywordRecords = await _elasticsearchVectorStore.QueryAsync<ElasticsearchTextVector>(
                collectionName,
            ...
            return await _elasticsearchVectorStore.HybridSearchAsync(
                collectionName,
                documentVector,
                queryVector,
                bm25Results,
        ...
        [HttpGet("ElasticsearchVectorStoreDelete")]
        public async Task ElasticsearchVectorStoreDelete([FromQuery] string id)
        {
            await _elasticsearchVectorStore.DeleteAsync("test_collection", id);
        }

        [HttpGet("ElasticsearchVectorStoreCollectionExists")]
        public async Task<bool> ElasticsearchVectorStoreCollectionExists()
        {
            return await _elasticsearchVectorStore.CollectionExistsAsync("test_collection");
        }

        [HttpGet("ElasticsearchVectorStoreDeleteCollection")]
        public async Task ElasticsearchVectorStoreDeleteCollection()
        {
            await _elasticsearchVectorStore.DeleteCollectionAsync("test_collection");
        }
    }
}
