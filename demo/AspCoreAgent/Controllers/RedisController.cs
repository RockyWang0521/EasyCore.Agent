...
namespace AspCoreAgent.Controllers
{
    ...
    public class RedisController : ControllerBase
    {
        ...
        private readonly IRedisVectorStore _redisVectorStore;
        private const string documentVector = "documentVector";

        public RedisController(QianwenAgent agent,
               DeepSeekAgent deepSeekAgent,
               IRedisVectorStore redisVectorStore)
        {
            _agent = agent;
        ...
        [HttpGet("RedisVectorStoreUpsert")]
        public async Task RedisVectorStoreUpsert()
        {
            var vectorCollectionDefinition = new RedisVectorCollectionDefinition
            {
                ScalarFields =
                {
            ...
            await _redisVectorStore.CreateCollectionAsync("test_collection", vectorCollectionDefinition);
...
让 AI 开发回归简单，让每一个 .NET 开发者都能轻松构建企业级 Agent 应用。
            ...
            foreach (var chunk in chunks)
            {
                ...
                var textVector = new RedisTextVector
                {
                ...
                await _redisVectorStore.UpsertAsync("test_collection", textVector);
            }
        }

        [HttpGet("RedisVectorStoreSearch")]
        public async Task<IReadOnlyList<RedisVectorSearchResult<RedisTextVector>>> RedisVectorStoreSearch()
        {
            ...
            var searchResults = await _redisVectorStore.VectorSearchAsync<RedisTextVector>(
        ...
        [HttpGet("RedisVectorStoreMmrSelector")]
        public async Task<List<MmrCandidate>> RedisVectorStoreMmrSelector()
        {
            var result = await _agent.EmbedAsync("easycore.agent支持哪些功能？");
        ...
        [HttpGet("RedisVectorStoreGet")]
        public async Task<RedisTextVector?> RedisVectorStoreGet([FromQuery] string? id)
        {
            const string collectionName = "test_collection";

            if (string.IsNullOrWhiteSpace(id))
            {
                var records = await _redisVectorStore.QueryAsync<RedisTextVector>(
                    collectionName,
                    limit: 1,
                    includeMetadata: true);
            ...
            return await _redisVectorStore.GetAsync<RedisTextVector>(
                collectionName,
                id,
                includeVector: true,
                vectorName: documentVector,
        ...
        [HttpGet("RedisVectorStoreQuery")]
        public async Task<IReadOnlyList<RedisTextVector>> RedisVectorStoreQuery()
        {
            return await _redisVectorStore.QueryAsync<RedisTextVector>(
                collectionName: "test_collection",
                filter: new RedisVectorFilter
                {
                    Conditions =
        ...
        [HttpGet("RedisVectorStoreHybridSearch")]
        public async Task<IReadOnlyList<RedisVectorSearchResult<RedisTextVector>>> RedisVectorStoreHybridSearch()
        {
            const string collectionName = "test_collection";
            var queryVector = await _agent.EmbedAsync("easycore.agent支持哪些功能？");

            var keywordRecords = await _redisVectorStore.QueryAsync<RedisTextVector>(
                collectionName,
            ...
            return await _redisVectorStore.HybridSearchAsync(
                collectionName,
                documentVector,
                queryVector,
                bm25Results,
        ...
        [HttpGet("RedisVectorStoreDelete")]
        public async Task RedisVectorStoreDelete([FromQuery] string id)
        {
            await _redisVectorStore.DeleteAsync("test_collection", id);
        }

        [HttpGet("RedisVectorStoreCollectionExists")]
        public async Task<bool> RedisVectorStoreCollectionExists()
        {
            return await _redisVectorStore.CollectionExistsAsync("test_collection");
        }

        [HttpGet("RedisVectorStoreDeleteCollection")]
        public async Task RedisVectorStoreDeleteCollection()
        {
            await _redisVectorStore.DeleteCollectionAsync("test_collection");
        }
    }
}
