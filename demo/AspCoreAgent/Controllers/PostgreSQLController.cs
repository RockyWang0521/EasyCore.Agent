...
namespace AspCoreAgent.Controllers
{
    ...
    public class PostgreSqlController : ControllerBase
    {
        ...
        private readonly IPostgreSqlVectorStore _postgreSqlVectorStore;
        private const string documentVector = "documentVector";

        public PostgreSqlController(QianwenAgent agent,
               DeepSeekAgent deepSeekAgent,
               IPostgreSqlVectorStore postgreSqlVectorStore)
        {
            _agent = agent;
            _deepSeekAgent = deepSeekAgent;
            _postgreSqlVectorStore = postgreSqlVectorStore;
        }

        [HttpGet("PostgreSqlVectorStoreUpsert")]
        public async Task PostgreSqlVectorStoreUpsert()
        {
            var vectorCollectionDefinition = new PostgreSqlVectorCollectionDefinition
            {
                ScalarFields =
                {
            ...
            await _postgreSqlVectorStore.CreateCollectionAsync("test_collection", vectorCollectionDefinition);
...
让 AI 开发回归简单，让每一个 .NET 开发者都能轻松构建企业级 Agent 应用。
            ...
            foreach (var chunk in chunks)
            {
                ...
                var textVector = new PostgreSqlTextVector
                {
                ...
                await _postgreSqlVectorStore.UpsertAsync("test_collection", textVector);
            }
        }

        [HttpGet("PostgreSqlVectorStoreSearch")]
        public async Task<IReadOnlyList<PostgreSqlVectorSearchResult<PostgreSqlTextVector>>> PostgreSqlVectorStoreSearch()
        {
            ...
            var searchResults = await _postgreSqlVectorStore.VectorSearchAsync<PostgreSqlTextVector>(
        ...
        [HttpGet("PostgreSqlVectorStoreMmrSelector")]
        public async Task<List<MmrCandidate>> PostgreSqlVectorStoreMmrSelector()
        {
            var result = await _agent.EmbedAsync("easycore.agent支持哪些功能？");
        ...
        [HttpGet("PostgreSqlVectorStoreGet")]
        public async Task<PostgreSqlTextVector?> PostgreSqlVectorStoreGet([FromQuery] string? id)
        {
            const string collectionName = "test_collection";

            if (string.IsNullOrWhiteSpace(id))
            {
                var records = await _postgreSqlVectorStore.QueryAsync<PostgreSqlTextVector>(
                    collectionName,
                    limit: 1,
                    includeMetadata: true);
            ...
            return await _postgreSqlVectorStore.GetAsync<PostgreSqlTextVector>(
                collectionName,
                id,
                includeVector: true,
                vectorName: documentVector,
        ...
        [HttpGet("PostgreSqlVectorStoreQuery")]
        public async Task<IReadOnlyList<PostgreSqlTextVector>> PostgreSqlVectorStoreQuery()
        {
            return await _postgreSqlVectorStore.QueryAsync<PostgreSqlTextVector>(
                collectionName: "test_collection",
                filter: new PostgreSqlVectorFilter
                {
                    Conditions =
        ...
        [HttpGet("PostgreSqlVectorStoreHybridSearch")]
        public async Task<IReadOnlyList<PostgreSqlVectorSearchResult<PostgreSqlTextVector>>> PostgreSqlVectorStoreHybridSearch()
        {
            const string collectionName = "test_collection";
            var queryVector = await _agent.EmbedAsync("easycore.agent支持哪些功能？");

            var keywordRecords = await _postgreSqlVectorStore.QueryAsync<PostgreSqlTextVector>(
                collectionName,
            ...
            return await _postgreSqlVectorStore.HybridSearchAsync(
                collectionName,
                documentVector,
                queryVector,
                bm25Results,
        ...
        [HttpGet("PostgreSqlVectorStoreDelete")]
        public async Task PostgreSqlVectorStoreDelete([FromQuery] string id)
        {
            await _postgreSqlVectorStore.DeleteAsync("test_collection", id);
        }

        [HttpGet("PostgreSqlVectorStoreCollectionExists")]
        public async Task<bool> PostgreSqlVectorStoreCollectionExists()
        {
            return await _postgreSqlVectorStore.CollectionExistsAsync("test_collection");
        }

        [HttpGet("PostgreSqlVectorStoreDeleteCollection")]
        public async Task PostgreSqlVectorStoreDeleteCollection()
        {
            await _postgreSqlVectorStore.DeleteCollectionAsync("test_collection");
        }
    }
}
